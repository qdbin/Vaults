        ...
        createClass(Mark3, [{
          ...
          value: function checkRanges(array) {
            ...
            if (!Array.isArray(array) || Object.prototype.toString.call(array[0]) !== "[object Object]") {
              this.log("标记范围（） will only accept an array of objects");
          ...
          value: function callNoMatchOnInvalidRanges(range, last) {
            ...
            if (range && typeof range.start !== "undefined") {
              ...
              this.log("忽略无效的范围：" + JSON.stringify(range));
              this.opt.noMatch(range);
            }
            return {
              start,
          ...
          value: function checkWhitespaceRanges(range, originalLength, string) {
            ...
            if (end > max) {
              ...
              this.log("结束范围已自动设置为最大值：" + max);
            }
            if (start < 0 || end - start < 0 || start > max || end > max) {
              valid = false;
              this.log("无效的范围：" + JSON.stringify(range));
              this.opt.noMatch(range);
            } else if (string.substring(start, end).replace(/\s+/g, "") === "") {
              valid = false;
              this.log("跳过仅包含空白字符的范围：" + JSON.stringify(range));
              this.opt.noMatch(range);
            }
            return {
              start,
          ...
          value: function markRanges(rawRanges, opt) {
            ...
            if (ranges && ranges.length) {
              this.log("开始标记以下范围：" + JSON.stringify(ranges));
...
var OP_MODE_INFO_DATA = {
  ["TextMode" /* Text */]: "文本模式",
  ["HighRestrictedMode" /* HighRestricted */]: "高限制模式",
  ["BalanceMode" /* Balance */]: "平衡模式",
  ["LowRestrictedMode" /* LowRestricted */]: "低限制模式",
  ["UnestrictedMode" /* Unrestricted */]: "无限制模式"
};
...
<table id="ophCompTable">
  ...
  <tr>
	<td><span> 低限制模式 </span></td>
	<td> 是 </td>
	<td> 是 </td>
	<td> 部分支持<sup>[2]</sup> </td>
	<td> 是 </td>
	<td> 否 </td>
	<td> 否 </td>
	<td> 是 </td>
...
<div><b>*</b>: <a href="https://web.dev/declarative-shadow-dom/">声明式 Shadow DOM (DSD)</a></div>
<div><b>#</b>: <a href="https://en.wikipedia.org/wiki/Content_Security_Policy">内容安全策略 (CSP)</a></div>
<div>[1]: 外部图片源将被内容安全策略 (CSP) 屏蔽。</div>
<div>[2]: <code>&lt;script&gt;</code> 标签内的脚本代码和外部脚本文件仍然不可执行。</div>
<div>[3]: 由于 Obsidian 的限制，外部脚本文件可能无法执行。</div>
<br />
</details>
...
<ol>
  <li><b>文本模式</b> - 强烈建议用于来自不受信任来源的文件！此模式几乎会清理掉所有的视觉效果、脚本代码和样式。同时，它会保留文本部分以及 HTML 布局元素，以便阅读 HTML 文件的内容。</li>
  <li><b>高限制模式</b> - 此模式推荐给需要更高安全性的用户。它会保留自定义元素，但会清理掉不安全的 HTML 元素、不安全的属性及其内容。外部图片源将被内容安全策略 (CSP) 屏蔽，图片仅能来自 HTML 文件本身。</li>
  <li><b>平衡模式</b> - 此模式是该插件的默认模式。它会保留所有自定义元素和 HTML 元素，但会清理掉不安全的属性及其内容。</li>
  <li><b>低限制模式</b> - 此模式不会清理任何内容，所有元素及其内容都将被保留。 <code>&lt;script&gt;</code> 标签内的脚本代码仍然不可执行，外部脚本文件也不可执行。 </li>
  
  <li><b style="color: red;">无限制模式</b> - 此模式 <b style="color: red;">非常危险</b> ，可能导致 Obsidian 应用程序崩溃， <b style="color: red; text-transform: uppercase;">Obsidian 和本插件对于切换到该模式不会承担任何责任或义务</b>。 它不会清理任何内容，所有元素及其内容都将被保留。Obsidian 应用程序本身可能会进行一些调整。外部脚本文件可能由于 Obsidian 的限制而无法执行。强烈建议正常使用时不要切换到此模式。<br />
    如果在切换到此模式后遇到问题，建议采取以下恢复步骤：
	<ul>
	  <li>返回到之前能正常打开的文件。</li>
	  <li><ins>删除已打开的出现问题的文件或将其移至回收站。否则，Obsidian 在重新启动后仍会尝试打开它。</ins></li>
	  <li>返回此设置页面并切换到另一种操作模式。</li>
	</ul>
	<br />
	有时您可能仍然看不到想要的内容，这时您应该检查 HTML 文件。此模式只是保留内容原样（仅<ins>调整外部链接锚点标签，使其在默认浏览器窗口中打开</ins>), 但文件可能包含自带的安全保护机制（例如 CSP），它们会阻止某些内容以避免 XSS 攻击。如果您在 HTML 文件中发现类似 <code>&lt;meta http-equiv="Content-Security-Policy" /&gt;</code> 的标签，意味着该文件受 CSP 机制保护。您可以：
	<ul>
	  <li>手动修改或删除该 CSP <code>&lt;meta&gt;</code> 标签。</li>
	  <li>更改原始网页保存应用程序的捕获设置，禁用 CSP 或其他相关设置，然后重新保存网页。</li>
	</ul>
  </li>
</ol>
</details>
...
var HtmlSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h1", { text: "HTML 阅读器设置" });
    containerEl.createEl("pre", { text: "\u203B 修改设置后请记得重新加载文件。" }).setAttribute("style", "color:red");
    containerEl.createEl("h2", { text: "常规设置" });
    const opModeSetting = new import_obsidian.Setting(containerEl);
    opModeSetting.setName("运行模式").setDesc("设置此插件的运行模式以保护用户和应用。").addDropdown((dropdown) => {
    ...
    bgColorSetting.setName("背景颜色").setDesc("强制设置 HTML 元素的背景颜色。").addColorPicker((picker) => {
    ...
    extraFileExtSetting.setName("额外文件扩展名").setDesc("使用用户自定义的文件扩展名打开 HTML 格式文件（逗号分隔的字符串列表）。修改此设置可能导致其他插件无法正常工作，因此请确保你明确知道自己在做什么。修改后请重启 Obsidian 应用！").addText(
      (val) => val.setValue(this.plugin.settings.extraFileExt).setPlaceholder("例如：xhtml, htm123").onChange(async (value) => {
    ...
    containerEl.createEl("h2", { text: "快捷键与手势设置" });
    containerEl.createEl("small", { text: `几乎所有的键盘快捷键都来源于 Obsidian 的全局快捷键设置，因此你应通过 \u2699 “设置” \u21E8 “快捷键” 页面进行修改。` });
    this.buildHotkeySettings();
    new import_obsidian.Setting(containerEl).setName("快速缩放页面").setDesc("使用 Ctrl + 鼠标滚轮 缩放文档（放大:  \u2191，缩小: \u2193），或使用触控板/触屏/触控面板的双指捏合手势（放大: \u2190 \u2192，缩小: \u2192 \u2190）。").addToggle((toggle) => {
  ...
  buildHotkeySettings() {
    ...
    hkSearch.setName("搜索页面文本").setDesc(`搜索当前文件。`);
    let hotkeyPairs = [
    ...
    if (!this.app.isMobile) {
      ...
      const hkZoomIn = new import_obsidian.Setting(containerEl).setName("放大页面").setDesc(`放大当前文件。`);
      ...
      const hkZoomOut = new import_obsidian.Setting(containerEl).setName("缩小页面").setDesc(`缩小当前文件。`);
      ...
      const hkZoomReset = new import_obsidian.Setting(containerEl).setName("重置页面缩放").setDesc(`重置当前文件缩放。`);
...
var Tp = 62;
var Ap = "FeatureRange";
var Lp = { kind: String, left: ["Identifier", "Number", "Dimension", "Ratio", "Function"], left模式对比: String, middle: ["Identifier", "Number", "Dimension", "Ratio", "Function"], right模式对比: [String, null], right: ["Identifier", "Number", "Dimension", "Ratio", "Function", null] };
function Mn() {
...
function Bn(e2 = "unknown") {
  ...
  return this.lookupNonWSType(0) !== 22 && (o = Xs.call(this), s = Mn.call(this)), this.skipSC(), this.eat(22), { type: "FeatureRange", loc: this.getLocation(t2, this.tokenStart), kind: e2, left: r, left模式对比: n, middle: i, right模式对比: o, right: s };
}
function Ep(e2) {
  this.token(21, "("), this.node(e2.left), this.tokenize(e2.left模式对比), this.node(e2.middle), e2.right && (this.tokenize(e2.right模式对比), this.node(e2.right)), this.token(22, ")");
}
var qn = {};
...
async function showError(e2) {
  ...
  notice.noticeEl.createEl("strong", { text: "HTML 阅读器错误" });
